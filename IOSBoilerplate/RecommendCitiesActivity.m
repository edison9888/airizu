...
@interface RecommendCitiesActivity ()
...
typedef NS_ENUM(NSInteger, NetRequestTagEnum) {
  // 2.4 推荐城市
  kNetRequestTagEnum_RecommendCity = 0
};
...
- (void)dealloc {
  ...
  // 当前界面一定要从这里进行网络的终止
  [[DomainProtocolNetHelperSingleton sharedInstance] cancelNetRequestByRequestIndex:_netRequestIndexForRecommendCity];
  _netRequestIndexForRecommendCity = IDLE_NETWORK_REQUEST_ID;
  
  //
  [_recommendCityTableCellUINib release];
  //
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
  if ([DeviceInformation isIPhone5]) {
    ...
    self = [super initWithNibName:@"RecommendCitiesActivity_iphone5" bundle:nibBundleOrNil];
  } else {
    self = [super initWithNibName:@"RecommendCitiesActivity" bundle:nibBundleOrNil];
  }
  if (self) {
...
- (void)viewDidUnload {
  ...
  [self setRecommendCityTableCellUINib:nil];
