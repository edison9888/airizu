...
+(id)activity {
  return [[[Activity alloc] init] autorelease];
}

+(id)activityFromSubclass:(Class)cls intent:(Intent *)intent {
  Activity *newActivity = [[[cls alloc] init] autorelease];
...
-(void)sendBroadcast:(Intent *)intent {
  ...
  = [NSNotification notificationWithName:intent.action object:self userInfo:userInfo];
