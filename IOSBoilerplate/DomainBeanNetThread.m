...
@property (nonatomic, assign) NetRequestEvent *requestEvent;
@property (nonatomic, assign) id networkCallback;
@end
...
- (void) dealloc {
  ...
  //[_requestEvent release];
  //[_networkCallback release];
  _requestEvent = nil;
...
+(id)domainBeanNetThreadWithNetRequestEvent:(NetRequestEvent *)requestEvent
                            networkCallback:(id)networkCallback {
  return [[[DomainBeanNetThread alloc] initWithNetRequestEvent:requestEvent networkCallback:networkCallback] autorelease];
}
...
- (void)main NS_AVAILABLE(10_5, 2_0) {	// thread body method
  ...
  PRPLog(@"-> %@", [netErrorBean description]);
