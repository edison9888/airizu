...
typedef NS_ENUM(NSInteger, AlertTypeEnum) {
  ...
  // 是否要清空本地缓存
  kAlertTypeEnum_ClearLocalCache
};
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
  if ([DeviceInformation isIPhone5]) {
    ...
    self = [super initWithNibName:@"AccountLoggedActivity_iphone5" bundle:nibBundleOrNil];
  } else {
    self = [super initWithNibName:@"AccountLoggedActivity" bundle:nibBundleOrNil];
  }
  if (self) {
...
- (void)viewDidUnload
{
  ...
  [self setLocalCacheSizeLabel:nil];
  [super viewDidUnload];
...
-(void)onResume {
  ...
  // 刷新 "缓存目录大小"
  [self updateLocalCacheSizeLabel];
}
...
#pragma mark -
#pragma mark UI相关
-(void)updateLocalCacheSizeLabel {
  // 刷新 "缓存目录大小"
  CGFloat size = [GlobalDataCacheForMemorySingleton sharedInstance].localCacheSize / 1024.0f / 1024.0f;
  _localCacheSizeLabel.text = [NSString stringWithFormat:@"清除缓存(%0.3fM)", size];
}
#pragma mark -
...
// "清除缓存"
- (IBAction)clearCacheButtonOnClickListener:(id)sender {
  UIAlertView *alert
  ...
  alert.tag = kAlertTypeEnum_ClearLocalCache;
  [alert show];
  [alert release];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
  ...
  switch (alertView.tag) {
    ...
    case kAlertTypeEnum_DownloadNewVersion:{// 更新新版本app
      if (buttonIndex != [alertView cancelButtonIndex]) {
